...
def test_admin_sso_principal_denied_when_no_allowlist(client, no_auth_env):
    # SSO principal present, ADMIN_EMAILS unset -> deny by default.
...
def test_admin_sso_principal_allowed_with_allow_all_override(client, monkeypatch, no_auth_env):
    monkeypatch.setenv("ADMIN_ALLOW_ALL_TENANT", "1")
    r = client.get("/admin/links", headers=SSO_HEADER)
    assert r.status_code == 200


def test_admin_sso_principal_match_is_case_insensitive(client, monkeypatch, no_auth_env):
    monkeypatch.setenv("ADMIN_EMAILS", "Thomas@WeChip.ch")
    r = client.get("/admin/links", headers={"X-MS-CLIENT-PRINCIPAL-NAME": "thomas@wechip.ch"})
    assert r.status_code == 200
